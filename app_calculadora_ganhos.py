...
def get_volumes(df, segmento, subcanal, anomes):
    ...
    vol_41 = df_f.loc[
        df_f["NM_KPI"].str.contains("4.1 - Usuários Únicos (CPF)", case=True, na=True) |
        df_f["NM_KPI"].str.contains("4.1 - Usuários Únicos (CPF)", case=False, na=False) &
        df_f["NM_KPI"].str.contains("CPF", case=False, na=False),
        "VOL_KPI"
    ].sum()
