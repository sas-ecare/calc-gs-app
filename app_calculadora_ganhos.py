...
def check_password():
    def password_entered():
        if st.session_state["password"] == "claro@123":  # senha fixa
...
volume_esperado = col1.number_input("📥 Insira o volume de transações", min_value=0, value=10000)
...
if st.button("🚀 Calcular Ganhos Potenciais"):
    ...
    # cálculo principal
    cr_ev = (volume_esperado / tx_trans_acessos) * cr_segmento * retido_pct
    ...
    valor_formatado = f"{cr_ev:,.0f}".replace(",", ".")
    st.success(f"✅ Volume de CR Evitado Estimado: **{valor_formatado}**")
    st.caption("Fórmula: Volume Acessos ÷ (Transações / Acessos) × CR Segmento × % Retido")
    ...
    insight_text = (
        ...
        f"- O volume total estimado de **CR evitado** para o subcanal selecionado foi de: **{valor_formatado}**.\n\n"
        f"- A estimativa do **CR evitado** para todos os canais seria de: **{total_ev_fmt}**.\n\n"
